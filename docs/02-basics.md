# 第二章：基本語法與型別系統

## 變數宣告

### JS vs Rust 變數對照

```javascript
// JavaScript
let x = 5;        // 可變
const y = 10;     // 不可變
var z = 15;       // 避免使用
```

```rust
// Rust
let x = 5;        // 不可變（預設！）
let mut y = 10;   // 可變（需要 mut）
const Z: i32 = 15; // 編譯時常數
```

### 重點差異

| 概念     | JavaScript        | Rust                      |
| -------- | ----------------- | ------------------------- |
| 預設行為 | `let` 可變        | `let` 不可變              |
| 可變變數 | `let`             | `let mut`                 |
| 常數     | `const`（執行時） | `const`（編譯時，需型別） |

### 變數遮蔽（Shadowing）

Rust 允許重新宣告同名變數，這在 JS 的同一作用域是不行的：

```rust
let x = 5;
let x = x + 1;      // OK！這是新的 x
let x = "hello";    // OK！甚至可以改變型別

// 這很實用，例如型別轉換：
let input = "42";
let input: i32 = input.parse().unwrap();  // 字串轉數字
```

## 基本型別

### 數字型別

Rust 的數字型別非常明確（不像 JS 只有 Number）：

```rust
// 整數（有正負號）
let a: i8 = 127;       // -128 到 127
let b: i16 = 32767;
let c: i32 = 2147483647;  // 預設整數型別
let d: i64 = 9223372036854775807;
let e: i128 = 170141183460469231731687303715884105727;

// 整數（無正負號）
let f: u8 = 255;       // 0 到 255
let g: u32 = 4294967295;
let h: usize = 100;    // 指標大小，用於索引

// 浮點數
let i: f32 = 3.14;
let j: f64 = 3.141592653589793;  // 預設浮點型別
```

### 型別推導

Rust 很聰明，大多數時候不需要寫型別：

```rust
let x = 42;        // 推導為 i32
let y = 3.14;      // 推導為 f64
let z = true;      // 推導為 bool
let s = "hello";   // 推導為 &str
```

### 字串：&str vs String

這是 JS 工程師常困惑的地方：

```rust
// &str - 字串切片（不可變，存在記憶體某處）
let s1: &str = "Hello";  // 字面值，編譯進程式

// String - 字串物件（可變，存在堆積上）
let s2: String = String::from("Hello");
let mut s3 = String::new();
s3.push_str("Hello");
```

**簡單記法**：

- `&str`：類似 JS 的字串字面值 `"hello"`
- `String`：類似 JS 的 `new String("hello")` 但更常用

```rust
// 轉換方式
let str_slice: &str = "hello";
let string: String = str_slice.to_string();  // &str -> String
let string: String = String::from("hello");  // 另一種方式
let back_to_slice: &str = &string;           // String -> &str
```

### 布林與字元

```rust
let t: bool = true;
let f: bool = false;

let c: char = 'A';      // 注意：單引號
let emoji: char = '😀';  // 支援 Unicode
```

## 函式定義

```javascript
// JavaScript
function add(a, b) {
    return a + b;
}

const multiply = (a, b) => a * b;
```

```rust
// Rust
fn add(a: i32, b: i32) -> i32 {
    a + b  // 沒有分號 = 回傳值
}

// 或明確 return
fn multiply(a: i32, b: i32) -> i32 {
    return a * b;  // 有分號
}

// 無回傳值
fn greet(name: &str) {
    println!("Hello, {}!", name);
}
```

### 重要觀念：表達式 vs 陳述式

```rust
fn example() -> i32 {
    let x = 5;      // 陳述式（有分號，無值）

    x + 1           // 表達式（無分號，有值）→ 這是回傳值
}

// 錯誤示範
fn wrong() -> i32 {
    let x = 5;
    x + 1;          // 加了分號變成陳述式，回傳 ()
}
```

## 控制流程

### if 表達式

```rust
// 基本用法
let number = 6;

if number % 2 == 0 {
    println!("偶數");
} else {
    println!("奇數");
}

// if 是表達式！可以賦值（類似 JS 的三元運算子）
let result = if number > 5 { "大" } else { "小" };
```

注意：Rust 的 if 條件不需要括號，但**必須是布林值**：

```rust
// JavaScript 可以
if (1) { console.log("truthy"); }

// Rust 不行！
if 1 { println!("錯誤！"); }  // 編譯錯誤
if 1 != 0 { println!("正確"); }  // 必須是 bool
```

### 迴圈

```rust
// loop - 無限迴圈
let mut count = 0;
let result = loop {
    count += 1;
    if count == 10 {
        break count * 2;  // break 可以回傳值！
    }
};

// while
let mut n = 3;
while n != 0 {
    println!("{}", n);
    n -= 1;
}

// for - 最常用
for i in 0..5 {           // 0, 1, 2, 3, 4
    println!("{}", i);
}

for i in 0..=5 {          // 0, 1, 2, 3, 4, 5（包含結尾）
    println!("{}", i);
}

// 遍歷陣列
let arr = [10, 20, 30];
for element in arr {
    println!("{}", element);
}
```

### match 表達式（超強的 switch）

```rust
let number = 3;

match number {
    1 => println!("一"),
    2 => println!("二"),
    3 => println!("三"),
    4 | 5 => println!("四或五"),  // 多個值
    6..=10 => println!("六到十"), // 範圍
    _ => println!("其他"),        // 預設（類似 default）
}

// match 也是表達式
let result = match number {
    1 => "one",
    2 => "two",
    _ => "other",
};
```

## JS vs Rust 語法對照表

| 功能           | JavaScript            | Rust                                |
| -------------- | --------------------- | ----------------------------------- |
| 變數（可變）   | `let x = 1`           | `let mut x = 1`                     |
| 變數（不可變） | `const x = 1`         | `let x = 1`                         |
| 函式           | `function f(a) {}`    | `fn f(a: i32) {}`                   |
| 箭頭函式       | `(a) => a + 1`        | `\|a\| a + 1`（閉包）               |
| 字串模板       | `` `Hello ${name}` `` | `format!("Hello {}", name)`         |
| 印出           | `console.log(x)`      | `println!("{}", x)`                 |
| 陣列           | `[1, 2, 3]`           | `[1, 2, 3]`（固定）或 `vec![1,2,3]` |
| 物件           | `{ a: 1, b: 2 }`      | `struct` 或 `HashMap`               |
| 空值           | `null / undefined`    | `Option<T>`                         |
| 三元運算       | `a ? b : c`           | `if a { b } else { c }`             |
| 型別檢查       | `typeof x`            | 編譯時確定，無需檢查                |

## 練習題

### 練習 1：變數與型別

```rust
fn main() {
    // 1. 宣告一個可變的整數變數 count，初始值為 0
    // 2. 將 count 加 1
    // 3. 印出 count

    // 你的程式碼
}
```

### 練習 2：函式

```rust
// 完成這個函式：計算兩數的平均值
fn average(a: f64, b: f64) -> f64 {
    // 你的程式碼
}

fn main() {
    let result = average(10.0, 20.0);
    println!("平均值: {}", result);  // 應該印出 15.0
}
```

### 練習 3：match 練習

```rust
// 完成這個函式：根據分數回傳等級
// 90-100: "A", 80-89: "B", 70-79: "C", 60-69: "D", 其他: "F"
fn grade(score: i32) -> &'static str {
    // 你的程式碼（使用 match）
}
```

### 練習 4：FizzBuzz

用 Rust 實作經典的 FizzBuzz：

- 印出 1 到 30
- 3 的倍數印 "Fizz"
- 5 的倍數印 "Buzz"
- 15 的倍數印 "FizzBuzz"

---

[← 上一章：環境設定](./01-getting-started.md) | [下一章：所有權與借用 →](./03-ownership.md)
